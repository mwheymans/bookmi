...
In this Chapter, we will apply more advanced multiple imputation models. With “advanced”, we mean multiple imputation models for Multilevel data, which are also called Mixed models. We start this Chapter with a brief introduction about multilevel data. Subsequently, we will shortly discuss the locations of missing data in Multilevel data. Finally, we will show examples of how to impute missing data in multilevel datasets using different imputation methods with mice. 
...
For the analysis of Multilevel data specific regression models have to be used. Logically, if Multilevel data contain missing values these models have to be implemented in the (multiple) imputation method.     
