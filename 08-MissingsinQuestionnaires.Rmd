...
In the methods tab the model type can be defined and the number of iterations (Figure 8.2), in the Constraints tab any constraints for the imputations can be specified (Figure 8.3) and in the Output tab the returned output can be selected and creating the iteration history to obtain convergence plots (Figure 8.4). Details about the options can be reviewed in Chapter 4 (paragraph 9). For imputing item scores, sometimes it may be feasible to constrain the minimum and maximum imputed values (when using the regression model type. Otherwise, imputations may contain values that can exceed the maximum and minimum score option of the items. 
...
Before analyzing the imputed data, the convergence of the imputations should be checked in iteration plots for each imputed item. In Chapter 4 paragraph 11 the procedure for creating iteration plots is described. In Figure 8.5a and Figure 8.5b you can observe iteration plots for two items. The plot in Figure 8.5a presents an iteration plot of a converged imputation, however the plot in Figure 8.5b presents a less well converged imputed item. This might be due to high correlations between that item and other items or variables in the imputation model. 
...
First we load the data in R and quickly inspect the dataset by displaying the first few rows. We can observe that the Total score has missing values when one or more item scores are missing (row 1 and row 2).
...
```{r}
imp <- mice(dat, m = 5, maxit = 10, pred = pred)
```
...
imp1$Total_Score <- rowSums(imp1[c(pate0("Item", 1:10))])
imp2 <- as.mids(imp1)
```
...
dat <- read_sav("data/ManyQuestionnaries.sav")
names(dat)
```
...
```{r}
dat1$Q1_parcel <- rowMeans(dat1[,c("Q1_I1","Q1_I2","Q1_I3","Q1_I4","Q1_I5","Q1_I6","Q1_I7","Q1_I8","Q1_I9","Q1_I10","Q1_I11","Q1_I12")], na.rm=TRUE)
dat1$Q2_parcel <- rowMeans(dat1[,c("Q2_I1","Q2_I2","Q2_I3","Q2_I4","Q2_I5","Q2_I6","Q2_I7","Q2_I8","Q2_I9","Q2_I10","Q2_I11","Q2_I12")], na.rm=TRUE)
dat1$Q3_parcel <- rowMeans(dat1[,c("Q3_I1","Q3_I2","Q3_I3","Q3_I4","Q3_I5","Q3_I6","Q3_I7","Q3_I8","Q3_I9","Q3_I10","Q3_I11","Q3_I12")], na.rm=TRUE)
dat1$Q4_parcel <- rowMeans(dat1[,c("Q4_I1","Q4_I2","Q4_I3","Q4_I4","Q4_I5","Q4_I6","Q4_I7","Q4_I8","Q4_I9","Q4_I10","Q4_I11","Q4_I12")], na.rm=TRUE)
dat1$Q5_parcel <- rowMeans(dat1[,c("Q5_I1","Q5_I2","Q5_I3","Q5_I4","Q5_I5","Q5_I6","Q5_I7","Q5_I8","Q5_I9","Q5_I10","Q5_I11","Q5_I12")], na.rm=TRUE)
...
```{r}
dat1_Q1 <- dat1[,c("Q1_I1","Q1_I2","Q1_I3","Q1_I4","Q1_I5","Q1_I6","Q1_I7","Q1_I8","Q1_I9","Q1_I10","Q1_I11","Q1_I12", 
                    "Q2_parcel", "Q3_parcel", "Q4_parcel", "Q5_parcel",
                    "Covariate1", "Covariate2", "Covariate3")]

```

For each questionnaire, the dataset can then be imputed. Since we already stored the variables that we need to use for the imputation per questionnaire, we can use that object (e.g dat1_Q1 for questionnaire 1) for the imputation. 

```{r}
imp_Q1 <- mice(dat1_Q1, m = 5, maxit = 10)
```
...
```{r}
dat1_imp <-data.frame(.imp=mi_Q1[,".imp"],
...
```{r}
dat1_imp$Q1_TS <- rowSums(dat1_imp[,c("Q1_I1","Q1_I2","Q1_I3","Q1_I4","Q1_I5","Q1_I6","Q1_I7","Q1_I8","Q1_I9","Q1_I10","Q1_I11","Q1_I12")], na.rm=FALSE)
dat1_imp$Q2_TS <- rowSums(dat1_imp[,c("Q2_I1","Q2_I2","Q2_I3","Q2_I4","Q2_I5","Q2_I6","Q2_I7","Q2_I8","Q2_I9","Q2_I10","Q2_I11","Q2_I12")], na.rm=FALSE)
dat1_imp$Q3_TS <- rowSums(dat1_imp[,c("Q3_I1","Q3_I2","Q3_I3","Q3_I4","Q3_I5","Q3_I6","Q3_I7","Q3_I8","Q3_I9","Q3_I10","Q3_I11","Q3_I12")], na.rm=FALSE)
dat1_imp$Q4_TS <- rowSums(dat1_imp[,c("Q4_I1","Q4_I2","Q4_I3","Q4_I4","Q4_I5","Q4_I6","Q4_I7","Q4_I8","Q4_I9","Q4_I10","Q4_I11","Q4_I12")], na.rm=FALSE)
dat1_imp$Q5_TS <- rowSums(dat1_imp[,c("Q5_I1","Q5_I2","Q5_I3","Q5_I4","Q5_I5","Q5_I6","Q5_I7","Q5_I8","Q5_I9","Q5_I10","Q5_I11","Q5_I12")], na.rm=FALSE)
...
dat1_imp2 <- as.mids(dat1_imp)
fit <- with(dat1_imp2, lm(Q1_TS~ Q2_TS+Q3_TS+Q4_TS+Q5_TS+Covariate1))
pool(fit)
```
...
To illustrate passive imputation we can use the same example data as was used for the parcel summary imputation. First, the entire dataset used to run an empty imputation in order to obtain the initial settings for the imputation for the predictor matrix and the methods 

```{r}
dat <- read_sav("data/ManyQuestionnaires")
imp0 <- mice(dat, maxit=0)
predmat2 <- imp0$pred
method2 <- imp0$method
```
...
In R code 8.17 you can see the code for the items of each questionnaire. Accordingly, for the imputation of the item scores of questionnaire1, we exclude the items of the other questionnaires and the total score for questionnaire 1, by changing the 1s in the Q1 item rows to 0s for the item columns of the other questionnaires and the total score of Q1. Note that the paste0 function is used to “paste” item number 1 to 12 to the first part of the item name “Q1_I”. 


```{r}
predmat2[paste0("Q1_I",1:12), c(paste0("Q2_I",1:12), paste0("Q3_I",1:12),paste0("Q4_I",1:12),paste0("Q5_I",1:12), "TS_Q1")] <- 0
predmat2[paste0("Q2_I",1:12), c(paste0("Q1_I",1:12), paste0("Q3_I",1:12),paste0("Q4_I",1:12),paste0("Q5_I",1:12), "TS_Q2")] <- 0
predmat2[paste0("Q3_I",1:12), c(paste0("Q1_I",1:12), paste0("Q2_I",1:12),paste0("Q4_I",1:12),paste0("Q5_I",1:12), "TS_Q3")] <- 0
predmat2[paste0("Q4_I",1:12), c(paste0("Q1_I",1:12), paste0("Q2_I",1:12),paste0("Q3_I",1:12),paste0("Q5_I",1:12), "TS_Q4")] <- 0
predmat2[paste0("Q5_I",1:12), c(paste0("Q1_I",1:12), paste0("Q2_I",1:12), paste0("Q3_I",1:12),paste0("Q4_I",1:12), "TS_Q5")] <- 0
``` 

For the total scores we exclude all variables, because the total scores are updated from the imputed item scores in the passive part of the imputation (see adaption of the imputation methods in R code 8.19). And for the covariate rows, we use only the total scores, so we exclude all columns except the total scores.

```{r}
predmat2[c("TS_Q1", "TS_Q2","TS_Q3","TS_Q4","TS_Q5"),] <- 0
predmat2[c("Covariate1", "Covariate2", "Covariate3"),!names(dat1)%in% c("TS_Q1", "TS_Q2","TS_Q3","TS_Q4","TS_Q5")] <- 0
```  

In mice, the imputation methods are by default set to “pmm” for all (continuous) variables, see Chapter 4 paragraph 5. In passive imputation, the total score should not be imputed directly. However, the total score should be the sum of the imputed item scores for that questionnaire. The method for imputation of the total score is therefore the calculation of the sum. 

```{r}
method2["TS_Q1"] <- "~I(Q1_I1+Q1_I2+Q1_I3+Q1_I4+Q1_I5+Q1_I6+Q1_I7+Q1_I8+Q1_I9+Q1_I10+Q1_I11+Q1_I12)"
method2["TS_Q2"] <- "~I(Q2_I1+Q2_I2+Q2_I3+Q2_I4+Q2_I5+Q2_I6+Q2_I7+Q2_I8+Q2_I9+Q2_I10+Q2_I11+Q2_I12)"
method2["TS_Q3"] <- "~I(Q3_I1+Q3_I2+Q3_I3+Q3_I4+Q3_I5+Q3_I6+Q3_I7+Q3_I8+Q3_I9+Q3_I10+Q3_I11+Q3_I12)"
method2["TS_Q4"] <- "~I(Q4_I1+Q4_I2+Q4_I3+Q4_I4+Q4_I5+Q4_I6+Q4_I7+Q4_I8+Q4_I9+Q4_I10+Q4_I11+Q4_I12)"
method2["TS_Q5"] <- "~I(Q5_I1+Q5_I2+Q5_I3+Q5_I4+Q5_I5+Q5_I6+Q5_I7+Q5_I8+Q5_I9+Q5_I10+Q5_I11+Q5_I12)"
```

The imputation of the data can now be performed, while using the adapted predictor matrix and imputation methods. In passive imputation we mostly need more iterations to reach convergence, in this example we used 30. The iteration plots can be checked for the passively imputed total scores. In R code 8.20 the code is displayed to obtain the trace plots for the total scores only and the iteration plots for the total scores of questionnaire 1 to 3 are displayed. 

```{r}
imp <- mice(dat1, predictorMatrix = predmat2, method=method2, m=15, maxit=30)
plot(imp[c("TS_Q1", "TS_Q2","TS_Q3","TS_Q4","TS_Q5")])
```
...
```{r}
fit <- with(imp, lm(TS_Q1~ TS_Q2+TS_Q3+TS_Q4+TS_Q5+Covariate1))
pool(fit)
```
